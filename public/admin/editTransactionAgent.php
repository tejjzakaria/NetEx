...
$id = $_GET['id'];
$sql = "SELECT * FROM agent_withdrawal_requests WHERE id='$id'";
$result = mysqli_query($conn, $sql);
$transactionData = mysqli_fetch_assoc($result);


$selectedUserID = $transactionData['agent'];  // Retrieve this from your parcel data
$message = "";
...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE agent_withdrawal_requests SET agent='$agent', tracking_id='$tracking_id',
amount='$amount', account='$account', comments='$comments', status='$status' WHERE id='$id'";
...
<head>
    <!--  Title -->
    <title>Admin - Transactions Agents</title>
    <!--  Required Meta Tag -->
                                ...
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item">
                                            <a class="text-muted " href="">Modifier Transaction</a>
                                        </li>
                            ...
                            <div class="card-body">
                                ...
                                <input type="hidden" name="id" value="<?php echo $transactionData['id'] ?>">
                                    <input type="hidden" name="tracking_id"
                                        value="<?php echo $transactionData['tracking_id'] ?>">
                                    <div class="">
                                        ...
                                        <div class="mb-3">
                                            <input type="text" class="form-control" placeholder="Compte" name="account"
                                                required value="<?php echo $transactionData['account'] ?>" />
                                        </div>
                                        <div class="mb-4">
                                            <select class="form-select" aria-label="Default select example"
                                                ...
                                                while ($row = mysqli_fetch_assoc($userDataP)) {
                                                    ...
                                                    $selected = ($full_name == $selectedUserID) ? 'selected' : '';
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <select class="form-select" aria-label="Default select example"
                                                ...
                                                <option value="En attente" <?php echo ($transactionData['status'] == 'En attente') ? 'selected' : ''; ?>>En attente</option>
                                                <option value="Succès" <?php echo ($transactionData['status'] == 'Succès') ? 'selected' : ''; ?>>Succès</option>
                                                <option value="échoué" <?php echo ($transactionData['status'] == 'échoué') ? 'selected' : ''; ?>>échoué</option>
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            <textarea type="text" name="comments" class="form-control"
                                                id="exampleInputtext" placeholder="Ajouter info ici"
                                                required><?php echo $transactionData['comments'] ?></textarea>
                                        </div>
