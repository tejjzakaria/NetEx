...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['csvFile']) && $_FILES['csvFile']['error'] == 0) {
        $csvFile = $_FILES['csvFile']['tmp_name']; // Temporary file path
        $file = fopen($csvFile, 'r'); // Open the file
        ...
        if ($file !== FALSE) {
            ...
            // Loop through CSV rows and insert data into the offers table
            while (($row = fgetcsv($file, 1000, ',')) !== FALSE) {
                // CSV columns: offer_name, offer_category, offer_price, offer_comission, link, offer_quantity, f_image
                ...
                // Prepare the SQL query to insert the data into the offers table
                $sql = "INSERT INTO parcels2 (userID, tracking_id, name, phone_number, city, product, agent, status, price, address, comments, state, comission) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                $stmt->bind_param("sssssssssssss", $userID, $tracking_id, $name, $phone_number, $city, $product, $agent, $status, $price, $address, $comments, $state, $comission);
                if (!$stmt->execute()) {
        ...
        ";
            ...
            $message = "<div class='alert alert-danger'>Error opening file.</div>";
        }
    } else {
...
<head>
    <!--  Title -->
    <title>Admin - Importer Colis</title>
    <!--  Required Meta Tag -->
                                ...
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item">
                                            <a class="text-muted " href="import_parcel.php">Ajouter Colis</a>
                                        </li>
                                        <li class="breadcrumb-item" aria-current="page">Importer colis</li>
                                    </ol>
                                </nav>
                            </div>
                            <div class="col-3">
                                ...
                                <div class="col-lg-6">
                                    <div class="mb-4">
                                        ...
                                        <input type="text" class="form-control mt-2" id="lastName1" name="tracking_id"
                                            placeholder="Entrer Tracking ID" value="<?php echo $tracking_id ?>"
                                            required />
                                    </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="mb-4">
                                        <label for="exampleInputPassword1"
                                            class="form-label fw-semibold">Fichier</label>
                                        <input type="file" class="form-control mt-2" id="lastName1" name="csvFile" required />
                                    </div>
                                </div>
                            </div>
                        </div>
