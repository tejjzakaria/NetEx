...
$sql = "SELECT user_stores.id, user_stores.store_name, user_stores.store_url, 
               user_stores.created_at, user_info.full_name 
        FROM user_stores 
        JOIN user_info ON user_stores.userID = user_info.id 
        ORDER BY user_info.full_name ASC";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $table_data .= '
        ...
        <td><input type="checkbox" class="row-select form-check-input contact-chkbox primary" value="' . $row['id'] . '"></td>
            ...
            <td>
                <span class="badge bg-primary fw-semibold fs-2">' . htmlspecialchars($full_name) . '</span>
            </td>
                                        ...
                                        <table class="table border text-nowrap customize-table mb-0 align-middle"
                                            ...
                                            <thead class="text-dark fs-4">
                                                <tr>
                                                    ...
                                                    <th>
                                                        <h6 class="fs-4 fw-semibold mb-0">Vendeur associé</h6>
                                                    </th>
                                                    <th>
